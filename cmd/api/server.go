...
func (app *application) serve() error {
	...
	// Start a background goroutine to listen to signals
	go func() {
		...
		// Log a "shutting down server" message when a signal is caught. Notice that we also call the
		// String method on the signal to get the signal name and include it in the log entry properties
		...
		// Call Shutdown on our server, passing in the context we just made. Shutdown will return nil if the graceful
		// shutdown was successful, or an error (which may happen because of a problem closing the listeners, or
	...
	// Otherwise, we wait to receive the return value from Shutdown on the shutdownError channel. If return value is an
	// error, we know that there was a problem with the graceful shutdown, and we return the error
	err = <-shutdownError
	if err != nil {
		return err
	}
